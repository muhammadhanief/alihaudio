...
import { Mic2, PlayCircle, Download, Clock, Code, Check, Search, ArrowUpDown, ArrowUp, ArrowDown } from "lucide-react";
...
export default function MyConversionsPage() {
    ...
    useEffect(() => {
        fetch(getApiUrl(`/api/my-conversions?_t=${Date.now()}`), { cache: "no-store" })
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <div className="space-y-5 animate-fade-in">
            {/* Header */}
            <div>
                <h1 className="text-2xl md:text-4xl font-black text-orange-950 tracking-tighter uppercase">Alih Audio Saya</h1>
                            ...
                            <div key={c.id} className="glass rounded-[24px] p-4 border border-orange-200/50 shadow-sm space-y-3">
                                ...
                                {c.judul && (
                                    <div className="flex items-start gap-2">
                                        <span className="text-sm font-black text-orange-950 leading-snug">{c.judul}</span>
                                    </div>
                                )}
                                ...
                                <div className="flex items-center justify-between gap-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div suppressHydrationWarning>
                                            ...
                                            <div className="text-[9px] font-bold text-orange-900/40 uppercase">
                                                {new Date(c.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}
                                            </div>
                                        </div>
                                    </div>
                                    <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider flex-shrink-0 ${c.mode === 'puter' ? 'bg-orange-600 text-white' : 'bg-amber-100 text-amber-700'}`}>
